...
hidden test cases) runs once the search is over, on the best candidate: the one
with the highest fitness in run/population.jsonl. The search never sees these
...
def main() -> None:
    ...
    best = max((json.loads(line) for line in lines if line.strip()), key=lambda row: row["fitness"])
