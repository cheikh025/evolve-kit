...
Everything needed to run EVOLVE on a new machine with the DeepSeek Harness (DSH) 0.1.7: the two plugins (one of them carries the Evolve Mode preset), and the benchmark tasks with their evaluators. Written for Linux, with DSH run from a source clone — including a Docker container reached over SSH.

```text
evolve-kit/
├── setup.sh                 installs DSH, the plugins (with the preset) and the benchmark evaluators (one command)
...
│   ├── dsh-candidate-builder/ directory-confined workers (web and runtime tools blocked for workers)
│   └── dsh-evolve-loop/     the evolve service, evolve_run, evolve_status, the runtime tools,
│                            and the "Evolve Mode" preset (presets/evolve.patch.yml, skills/)
└── tasks/
...
| pnpm | 11.7.0 | `pnpm -v` |
| git | any | `git --version` |
| DeepSeek Harness | a source clone pinned to **0.1.7-rc.1** (tag `dsh-v0.1.7-rc.1`). `setup.sh` clones and builds it when `$DSH_REPO` is missing, so you do not have to — never install `@deepseek-ai/dsh` from npm instead | `node -p "require('$DSH_REPO/apps/cli/package.json').version"` |
...
That one command does the rest: it checks the requirements, clones and builds DSH 0.1.7-rc.1 at `$DSH_REPO` (or uses the clone already there), installs both plugins into `$DSH_HOME` — `dsh-evolve-loop` brings the Evolve Mode preset with it — and verifies the result. It is safe to run again.
...
If you already have a DSH clone, point `DSH_REPO` at it first. If it is not on 0.1.7-rc.1, commit or stash your work in it so setup can switch it.
...
| DSH version | `node -p "require('$DSH_REPO/apps/cli/package.json').version"` | `0.1.7-rc.1`; DSH refuses the plugins on another version (outside `>=0.1.7-rc.1 <0.2.0`) |
...
| preset installed | `ls "$DSH_HOME/profiles/web/node_modules/dsh-evolve-loop/presets"` | `evolve.patch.yml` |
...
| runtime changes work | in an Evolve Mode session, ask it to call `evolve_plugins` | `{ "plugins": [], "providers": { ... } }` |
...
- **`evolve_define` or `evolve_activate` says the runner is missing** — the host composition does not mount `@deepseek-ai/dsh-cordis-host-runner`. The DSH web profile mounts it by default; use the `web` profile.
...
- **Evolve Mode is not listed, or cannot be selected** — DSH refused `dsh-evolve-loop` or its preset. DSH prints the reason at startup and saves full startup diagnostics; an `incompatible-version` refusal means the DSH clone is not 0.1.7.
- **`evolve_status` or a worker fails** — the plugins are built and tested against DSH 0.1.7-rc.1. On another version something they use may have changed; keep the exact error, since the plugin source may then need updating for that version.
