"""Python port of the pinned AHC002 input generator."""
...
def generate(seed):
    ...
    start = [rng.integer(0, 50, 64), rng.integer(0, 50, 64)]
    tiles = [[i * 50 + j for j in range(50)] for i in range(50)]
    positions = [(i, j) for i in range(50) for j in range(50)]
    ...
    for i, j in positions:
        ...
        for di, dj in directions:
            x, y = i + di, j + dj
            if 0 <= x < 50 and 0 <= y < 50 and (x, y) not in used:
                tiles[x][y] = tiles[i][j]
                used.update([(x, y), (i, j)])
                break
    ids = {}
    for row in tiles:
        for tile in row:
            ids.setdefault(tile, len(ids))
    tiles = [[ids[tile] for tile in row] for row in tiles]
    points = [[rng.integer(0, 100) for _ in range(50)] for _ in range(50)]
    return rows([start]) + rows(tiles) + rows(points)
