"""Python port of the pinned AHC004 input generator."""
...
def generate(seed):
    ...
    grid = [[chr(65 + rng.integer(0, 8, 8)) for _ in range(20)] for _ in range(20)]
    length, count = rng.integer(4, 11), rng.integer(400, 801)
    strings = []
    for _ in range(count):
        i, j, direction, k = rng.integer(0, 20), rng.integer(0, 20), rng.integer(0, 2), rng.integer(length - 2, length + 3)
        strings.append("".join(grid[(i + (direction == 1) * p) % 20][(j + (direction == 0) * p) % 20] for p in range(k)))
    return f"20 {count}\n" + "\n".join(strings) + "\n"
