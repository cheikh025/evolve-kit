"""Python port of the pinned AHC005 input generator."""
...
def generate(seed):
    Random.seed_from_u64(seed)  # Validate before applying the upstream seed XOR.
    rng = Random.seed_from_u64(seed ^ 10)
    n = rng.integer(25, 36) * 2 - 1
    grid = [["#"] * n for _ in range(n)]
    for _ in range(rng.integer(n * 2, n * 4 + 1, 64)):
        direction = rng.integer(0, 2)
        i, j, half, weight = rng.integer(0, (n + 1) // 2) * 2, rng.integer(0, n), rng.integer(3, 11), str(rng.integer(5, 10))
        for k in range(max(j - half, 0), min(j + half, n - 1) + 1):
            x, y = (i, k) if direction == 0 else (k, i)
            grid[x][y] = weight
    seen, largest = set(), []
    for i in range(n):
        for j in range(n):
            if grid[i][j] == "#" or (i, j) in seen:
                continue
            component, stack = [], [(i, j)]
            seen.add((i, j))
            while stack:
                ...
                for dx, dy in DIR.values():
                    a, b = x + dx, y + dy
                    if 0 <= a < n and 0 <= b < n and grid[a][b] != "#" and (a, b) not in seen:
                        seen.add((a, b))
                        stack.append((a, b))
    ...
    largest = set(largest)
    while True:
        i, j = rng.integer(0, n), rng.integer(0, n)
        if (i, j) in largest:
            break
    return f"{n} {i} {j}\n" + "".join("".join(grid[x][y] if (x, y) in largest else "#" for y in range(n)) + "\n" for x in range(n))
